...
export async function downloadReportTextAsPdf(params: {
  ...
  pdf.setFontSize(16);
  pdf.text("Radix Radiology", margin, y);
  y += 7;
  pdf.setFontSize(10);
...
export function exportReportToPdf(params: {
  ...
  printWindow.document.write(`
    ...
    <html>
      <head>
        <title>Radix Report ${escapeHtml(params.accession)}</title>
        <style>
      ...
      <body>
        ...
        <div class="footer">Electronically generated by Radix. Final clinical correlation required.</div>
      </body>
    </html>
  `);
